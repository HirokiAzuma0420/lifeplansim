...
function runSimulation(params: InputParams): YearlyData[] {
  ...
  for (let i = 0; currentAge <= params.endAge; i++, currentAge++) {
    ...
    yearlyData.push({
      ...
      applianceExpense: applianceExpense,
      childExpense: childExpense,
      marriageExpense: marriageExpense,
      careExpense: careExpense,
      retirementExpense: retirementExpense,
      savings: savings,
      totalInvestment: totalInvestmentOutflow,
      cashFlow: cashFlow,
      nisa: { principal: nisa.principal, balance: nisa.balance },
      ideco: { principal: ideco.principal, balance: ideco.balance },
      taxable: { principal: taxable.principal, balance: taxable.balance },
      totalAssets: totalAssets,
      assetAllocation: {
        cash: savings,
        investment: taxable.balance,
        nisa: nisa.balance,
        ideco: ideco.balance,
      },
