...
// useFormState の validateSection が各セクションで仕様どおりに動作するかを確認する
describe('TC-FORM-002: バリデーションロジック', () => {
  ...
  // --- HOUSING セクション ---
  it('HOUSING セクション: 賃貸と購入意向・購入プラン・リフォームの条件付き必須が効く', () => {
    const { result } = renderHook(() => useFormState(), { wrapper });
  ...
  it('RETIREMENT_PLAN セクション: デフォルト状態では通る（退職年齢などの必須項目）', () => {
    const { result } = renderHook(() => useFormState(), { wrapper });
  ...
  it('RETIREMENT_INCOME セクション: 再雇用関連の条件付き必須も RETIREMENT_INCOME で検証される', () => {
    const { result } = renderHook(() => useFormState(), { wrapper });
