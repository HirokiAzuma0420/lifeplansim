  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-4 p-4 border border-blue-200 bg-blue-50 rounded-md">
        <label className="block text-gray-700 text-sm font-bold mb-2">利回りシナリオの選択</label>
          ...
          <div>
            <label className="inline-flex items-center">
              <input
                ...
                required
              />
              <span className="ml-2 font-semibold">ランダム変動</span>
            </label>
            <p className="text-xs text-gray-600 mt-1 ml-6">
              市場の好不調や暴落も考慮した、より現実的なストレステストを兼ねたシナリオです。
            </p>
          </div>
          <div>
            <label className="inline-flex items-center">
              <input
                ...
                required
              />
              <span className="ml-2 font-semibold">固定利回り</span>
            </label>
            <div className={`accordion-content ${formData.interestRateScenario === '固定利回り' ? 'open' : ''}`}>
              <div className="mt-2 ml-6">
                <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="fixedInterestRate">固定利率</label>
                <div className="flex max-w-xs">
                  <input type="number" id="fixedInterestRate" name="fixedInterestRate" value={formData.fixedInterestRate} onChange={handleInputChange} step="0.1" className="shadow appearance-none border rounded-l w-full py-2 px-3 text-gray-700" />
                  <span className="inline-flex items-center px-3 rounded-r border border-l-0 bg-gray-50 text-sm">%</span>
                </div>
                <p className="text-xs text-gray-600 mt-2">この設定を有効にすると投資セクションの個別利率は無視されます。</p>
              </div>
            </div>
          </div>
        </div>
