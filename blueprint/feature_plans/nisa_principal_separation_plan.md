...
- ユーザーがNISA口座の「評価総額」と「評価損益率」を個別に入力できるようにする。
...
2.  **評価損益率入力**:
    - 新たに「評価損益率」の入力エリアを設ける。
    - 評価損益率のプラス/マイナスを直感的に選択できるよう、ラジオボタンまたはトグルスイッチ（`+` / `-`）を設置する。
    - 数値入力フィールドを設け、小数点以下の入力も可能とする。
3.  **State管理の更新 (`useFormState.ts`)**:
    - `formState`に評価損益率と符号を保持するための新しいstateを追加する。
...
1.  **元本算出ロジック**:
    - 以下の計算式を用いて、評価総額と評価損益率から元本を算出する。
      ```
      元本 = 評価総額 / (1 + (評価損益率 / 100))
      ```
    - 評価損益率がマイナスの場合は、符号を適切に処理する。
2.  **APIパラメータへの反映**:
...
1.  **資産内訳テーブル (`AssetTable.tsx`)**:
    - テーブルのカラムは現状通り「NISA積立」「iDeCo積立」とし、本変更によって初年度の積立金額が元本ベースになる
2.  **各種チャートコンポーネント**:
    - `TotalAssetChart`や`AssetPieChart`などで、NISA資産の表示粒度については、これまでどおり評価額ベースとする。
...
1.  **単体テスト**:
    - `api-adapter.ts`に追加した元本算出ロジックについて、評価損益率がプラス、マイナス、ゼロの場合のテストケースを作成し、計算が正確であることを確認する。
2.  **コンポーネントテスト**:
    - `InvestmentSection.tsx`で、評価損益率の入力がstateに正しく反映されることを確認する。
    - `AssetTable.tsx`で、元本と評価額が分離して表示されることを確認する。
3.  **E2Eテスト**:
    - NISA口座情報を入力（例：評価額120万円、評価損益率+20%）し、シミュレーションを実行する。
